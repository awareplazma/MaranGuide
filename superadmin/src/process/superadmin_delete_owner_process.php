...
try {
    ...
    $stmt = $conn->prepare("SELECT admin_profile_picture FROM admin WHERE admin_id = ?");
    if (!$stmt) {
    ...
    if ($attraction && !empty($attraction['admin_profile_picture'])) {
        $image_path = $attraction['admin_profile_picture'];
